...
//Note you will use "basic_backend" in this JavaScript code a few times to call the backend
import { basic_backend } from "../../declarations/basic_backend";
...
document.addEventListener('DOMContentLoaded', async (e) => {
  ...
  const question = await basic_backend.getQuestion();
  ...
  const voteCounts = await basic_backend.getVotes();
...
//via "await basic_backend.vote(selectedOption)"
pollForm.addEventListener('submit', async (e) => {
  ...
  const updatedVoteCounts = await basic_backend.vote(checkedValue);
...
resetButton.addEventListener('click', async (e) => {
    ...
    await basic_backend.resetVotes();
    const voteCounts = await basic_backend.getVotes();
    updateLocalVoteCounts(voteCounts);
